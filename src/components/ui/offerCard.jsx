...
const OfferCard = ({ offer, featured = false }) => {
  ...
  return (
    ...
    >
      ...
      {/* ── title ── */}
      <div className='mb-3'>
        <h3 className='text-gray-900 font-semibold text-base mb-1'>{offer.title}</h3>
      ...
      <div className='space-y-2 mb-5'>
        {offer.benefits.map((b, i) => (
          <div key={i} className='flex items-start gap-2'>
            ...
            <p className='text-sm text-gray-600 leading-tight'>{b.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className='border-t pt-4 flex items-end justify-between'>
        <div>
          {offer.originalPrice && (
            <p className='text-gray-300 text-xs line-through'>{offer.originalPrice}</p>
          )}
          <div className='flex items-center gap-2'>
            <p className='text-lg font-bold text-gray-900'>{offer.finalPrice}</p>
            {offer.savings && (
              <span className='text-xs font-semibold text-green-600'>Save {offer.savings}</span>
            )}
          </div>
        </div>

        {/* ── Protected claim button ── */}
        <Button
          onClick={claimed ? undefined : protect(handleClaim)}
